...
def result_record(model: str, question: str, round_number: int, started: str,
    ...
    return {
        ...
        "capture_label": (
            "Scrapling 隐身浏览器网页直采"
            if capture_mode.startswith("scrapling") else "无头浏览器网页直采"
        ),
    ...
    try:
        for model in models:
            ...
            try:
                ...
                if not ready.get("ok"):
                    raise RuntimeError(ready.get("message") or f"{model} 网页登录状态不可用")
...
def main() -> int:
    ...
    while True:
        try:
            response = api.post("/api/worker/claim", {"worker_id": args.worker_id})

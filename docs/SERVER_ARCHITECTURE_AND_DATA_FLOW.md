...
服务端固定创建豆包、腾讯元宝、文心一言三模型各 8 轮，共 24 步，返回随机 32 位十六进制 `report_key`、`/geo/<report_key>` 路径和任务对象。同一 IP 五秒内重复提交会被拒绝。报告不使用品牌名或产品名作为路径。
...
先检查 `/geo/api/health` 和 `/geo/api/diagnosis`；再创建 1 模型 × 1 轮管理任务验证领取、心跳、落盘、分析、回传和完成；最后执行 3 模型 × 8 轮客户诊断。
